...
test('Início usa módulos com respiro sem romper os contratos dinâmicos', () => {
  ...
  assert.match(dashboard, /\.product-workspace \{[^}]*display:flex;[^}]*gap:42px;/);
  assert.match(dashboard, /class="workspace-section-label"[\s\S]*?>Seu currículo<[\s\S]*?Abrir currículo/);
...
test('tipografia principal permanece legível sem depender de microtexto', () => {
  assert.match(dashboard, /\.pba-title \{[^}]*font-size:34px;/);
  assert.match(dashboard, /\.pba-desc \{[^}]*font-size:15px;/);
  assert.match(dashboard, /\.pv-title \{[^}]*font-size:29px;/);
  assert.match(dashboard, /\.pv-desc \{[^}]*font-size:15px;/);
  assert.match(dashboard, /\.profile-value-tools \.fer-nome \{ font-size:14px; \}/);
  assert.match(dashboard, /\.bus-heading h3 \{[^}]*font-size:27px;/);
  assert.match(dashboard, /\.product-workspace \.opp-list-title-t \{[^}]*font-size:27px;/);
  assert.match(dashboard, /\.product-workspace \.opp-item-cargo \{ font-size:15px; \}/);
});
...
test('aderência da vaga usa anel próprio e não compete com a completude', () => {
  assert.match(dashboard, /ringFill\.setAttribute\('stroke-dashoffset'/);
  assert.match(dashboard, /\.pba-ring-wrap svg \{ display:block;/);
  assert.match(dashboard, /\.pba-score-row::after \{ display:none; \}/);
...
test('as quatro ferramentas continuam dinâmicas e acionáveis', () => {
  ...
  assert.doesNotMatch(dashboard, /<span class="fer-arrow"/);
