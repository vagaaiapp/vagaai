...
export async function isWithinLimit({ key, limit, windowMs }) {
  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) return true;
  try {
    const row = await getLimitRow(key);
    if (!row || Date.now() - new Date(row.first_seen).getTime() >= windowMs) return true;
...
export async function countLimit({ key, windowMs }) {
  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) return true;
  try {
    const row = await getLimitRow(key);
    ...
    if (row && Date.now() - new Date(row.first_seen).getTime() < windowMs) {
      const r = await fetch(
        `${SUPABASE_URL}/rest/v1/ip_rate_limits?ip=eq.${encodeURIComponent(key)}`,
        {
          method: 'PATCH',
          headers: headers(),
    ...
    const r = await fetch(`${SUPABASE_URL}/rest/v1/ip_rate_limits?on_conflict=ip`, {
      method: 'POST',
